...
def main():
    ...
    relative_data_path = 'data/processed/microbiome_filtered.csv'
    ...
    # Load metadata
    relative_data_path = 'data/processed/metadata_original.csv'
    out_dir_name = relative_data_path.split('/')[-1].split('.')[0] # same as dataset name
    df = pd.read_csv(relative_data_path)
    obj2 = BioFlowMLClass(df, out_dir_name = out_dir_name, lang = 'lv')
    
    # Generate merged synthetic dataset
    relative_merged_synthetic_data_path = generate_merged_synthetic_data(obj, obj2, 'sample_id', evaluate=False)
